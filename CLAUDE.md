...
- **Strom stránok (kostra, bez otázok):** `src/lib/dotaznik/strom.ts` (11 zjednodušených modulov → témy → 17-sekciová kostra + helpery + `cesta.*`). Zdieľané UI `_ui.tsx`. Screening `chcem` (Áno/Ešte nie/Nie) je **zlúčený do stránky témy** (`t/[tema]/page.tsx` → `_screening.tsx`); modulový screening zrušený; `/chcem`, `/temy` sú `redirect()`.
- **Obsah tém — 2 režimy:** (a) generický „section walker" `otazky.ts` + `_odpovede.tsx` (pevná kostra sekcií); (b) **„kniha + dotazník" data-driven** — `src/lib/dotaznik/obsah/**` (`typ.ts` schéma blokov + `podmienka` vetvenie + `m`/`z` gender varianty, `index.ts` registr) + renderer `_kniha.tsx` (+ výber pohlavia, autosave). Téma v registri → screening vedie na `/t/[tema]/kniha`, preskočí `rola`. Prvá 1:1 téma: `obsah/face-sitting.ts`.
