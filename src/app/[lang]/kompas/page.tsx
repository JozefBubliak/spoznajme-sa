...
export const metadata = {
  ...
  description: "Krátke vety a minipostupy do bežných situácií. Vyber si pre koho alebo otvor tému.",
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 space-y-8">
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground max-w-2xl">Krátke, použiteľné vety a kroky do bežných situácií. Vyber si pre koho alebo otvor tému.</p>
      </header>

      <section>
        <h2 className="text-xl font-semibold mb-3">Pre koho</h2>
        <div className="grid md:grid-cols-2 gap-4">
          <Link href={go("/kompas/rodic-dieta")} className="rounded-xl border p-5 hover:bg-muted transition block">
            <h3 className="font-medium">Rodič–dieťa</h3>
            <p className="text-sm text-muted-foreground mt-1">Emócie, hranice, dohody, zmeny. Primerané veku, bez moralizovania.</p>
          </Link>
          <Link href={go("/kompas/pary")} className="rounded-xl border p-5 hover:bg-muted transition block">
            <h3 className="font-medium">Páry</h3>
            <p className="text-sm text-muted-foreground mt-1">Vyjadrenie potrieb, prevencia hádok, zdravý konflikt, rituály spojenia.</p>
          </Link>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {topics.map(t => (
            <Link key={t.slug} href={go(`/kompas/tema/${t.slug}`)} className="rounded-lg border p-4 hover:bg-muted transition block">
