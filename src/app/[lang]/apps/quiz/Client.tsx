...
export default function QuizAdminClient({ lang }: Props) {
  ...
  const autoStartAttempts = useRef(new Set<number>())
  ...
  const ensureInitialRoundStarted = useCallback(
    async (latestDiagnostics?: RoundDiagnostic[]) => {
      ...
      const sorted = [...source].sort((a, b) => a.index - b.index)
      const seenIndices = new Set(sorted.map((diag) => diag.index))

      if (totalRounds) {
        for (let idx = 0; idx < totalRounds; idx += 1) {
      ...
      const alreadyProgressing = sorted.find((diag) => {
        const status = normalizeRoundStatus(diag.status)
        return ['running', 'locked', 'results', 'complete'].includes(status)
      })
      if (alreadyProgressing) return

      const candidate = sorted.find((diag) => diag.index === 0) ?? sorted[0]
      if (!candidate) return

      if (autoStartAttempts.current.has(candidate.index)) return

      const status = normalizeRoundStatus(candidate.status)
      if (!['ready', 'setup'].includes(status)) return

      autoStartAttempts.current.add(candidate.index)
      const started = await startRound({
        index: candidate.index,
        diagnosticsOverride: sorted,
        silent: true,
      })
      if (!started) {
        autoStartAttempts.current.delete(candidate.index)
      }
    },
    [diagnostics, gameCode, gameStatus, startRound, totalRounds],
  )
