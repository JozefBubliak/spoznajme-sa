-- ═══════════════════════════════════════════════════════════════
-- HERD VOTE — OPRAVENÁ MIGRÁCIA (bez duplicate chýb)
-- deeptalks.eu · Verzia 3.1
-- 
-- Prečo chyba? SQL bol čiastočne spustený → niektoré kategórie
-- (napr. "Videohry") sa už vložili. 
-- Oprava: ON CONFLICT (name) DO NOTHING
-- = vloží len to čo chýba, zvyšok ticho preskočí. Žiadna chyba.
-- ═══════════════════════════════════════════════════════════════

-- KROK 1: Pridaj stĺpce (bezpečné, IF NOT EXISTS)
-- ─────────────────────────────────────────────────
...
-- KROK 2: UPDATE existujúcich 23 (podľa ID — vždy bezpečné)
-- ──────────────────────────────────────────────────────────
...
-- KROK 3: BEZPEČNÝ INSERT nových kategórií
-- ON CONFLICT (name) DO NOTHING = preskočí už existujúce, vloží zvyšok
-- Môžeš spustiť koľkokrát chceš — nikdy nevyhodí chybu duplicate
-- ──────────────────────────────────────────────────────────────
INSERT INTO herd_categories
  (name, is_active, display_order, lang, sort_order, locale, icon, group_tag, description_sk, source_hint)
VALUES
...
('Revolúcie & Hnutia',              true,100,'sk',1670,'sk','✊','historia','Francúzska revolúcia, Velvet Revolution',             'Trivial Pursuit'),
...
('Hodnoty & Presvedčenia',          true,100,'sk',2910,'sk','🌍','deeptalks','Čo je pre teba najdôležitejšie?',                    'DeepTalks hlboké ⭐')

ON CONFLICT (name) DO NOTHING;  -- ← TOTO je kľúč. Preskočí existujúce, vloží zvyšok.
...
-- Celkový počet
SELECT COUNT(*) AS celkom_kategorii FROM herd_categories WHERE is_active = true;

-- Rozdelenie podľa skupín
SELECT
  COALESCE(group_tag, '(bez group_tag)') AS skupina,
  COUNT(*) AS pocet,
  COUNT(*) FILTER (WHERE icon = '❓') AS bez_ikony
FROM herd_categories
WHERE is_active = true
GROUP BY group_tag
ORDER BY pocet DESC;

-- Kategórie bez ikony (treba UPDATE)
SELECT id, name FROM herd_categories
