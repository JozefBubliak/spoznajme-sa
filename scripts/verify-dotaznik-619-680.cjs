...
const map = JSON.parse(fs.readFileSync(path.join(root, 'docs/dotaznik-zdroj-619-680.json'), 'utf8'))
const files = ["predohra-naladenie","roleplay"]
const bank = new Map()
...
assert.equal(map.paragraphs.length, 62)
let choices = 0, own = 0
for (const [i, p] of map.paragraphs.entries()) {
  assert.equal(p.p, i + 619, 'Chýbajúci alebo opakovaný odsek')
...
assert.equal(choices, 30)
assert.equal(own, 3)
console.log(`PASS: 62 odsekov P619–680, ${choices} volieb, ${own} vlastných odpovedí, oba rodové texty a platné referencie.`)
