...
const map = JSON.parse(fs.readFileSync(path.join(root, 'docs/dotaznik-zdroj-541-618.json'), 'utf8'))
const files = ["zmyslova-hra","predohra-naladenie","dlhodoba-intimita","bozky-dotyky"]
const bank = new Map()
...
assert.equal(map.paragraphs.length, 78)
let choices = 0, own = 0
for (const [i, p] of map.paragraphs.entries()) {
  assert.equal(p.p, i + 541, 'Chýbajúci alebo opakovaný odsek')
...
assert.equal(choices, 40)
assert.equal(own, 10)
console.log(`PASS: 78 odsekov P541–618, ${choices} volieb, ${own} vlastných odpovedí, oba rodové texty a platné referencie.`)
