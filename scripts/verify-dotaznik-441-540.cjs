...
const map = JSON.parse(fs.readFileSync(path.join(root, 'docs/dotaznik-zdroj-441-540.json'), 'utf8'))
const files = ['zmyslova-hra']
const bank = new Map()
...
assert.equal(map.paragraphs.length, 100)
let choices = 0, own = 0
for (const [i, p] of map.paragraphs.entries()) {
  assert.equal(p.p, i + 441, 'Chýbajúci alebo opakovaný odsek')
...
assert.equal(choices, 96)
assert.equal(own, 24)
console.log(`PASS: 100 odsekov P441–540, ${choices} volieb, ${own} vlastných odpovedí, oba rodové texty a platné referencie.`)
