...
const map = JSON.parse(fs.readFileSync(path.join(root, 'docs/dotaznik-zdroj-001-260.json'), 'utf8'))
const files = ['predohra-naladenie', 'dlhodoba-intimita', 'miesta-prostredie']
const bank = new Map()
...
for (const [i, p] of map.paragraphs.entries()) {
  assert.equal(p.p, i + 1, 'Chýbajúci alebo opakovaný odsek')
...
// Nové otázky nezneužívajú historické ID s odlišným významom.
assert(!bank.has('predohra-naladenie:pri_oblecenie'))
