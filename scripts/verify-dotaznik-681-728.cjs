...
const map = JSON.parse(fs.readFileSync(path.join(root, 'docs/dotaznik-zdroj-681-728.json'), 'utf8'))
const files = ["bdsm"]
const bank = new Map()
...
assert.equal(map.paragraphs.length, 48)
let choices = 0, own = 0
for (const [i, p] of map.paragraphs.entries()) {
  assert.equal(p.p, i + 681, 'Chýbajúci alebo opakovaný odsek')
...
assert.equal(own, 12)
console.log(`PASS: 48 odsekov P681–728, ${choices} volieb, ${own} vlastných odpovedí, oba rodové texty a platné referencie.`)
